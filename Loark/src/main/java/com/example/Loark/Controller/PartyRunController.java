...
public class PartyRunController {
    ...
    @Data
    static class CreatePartyRunRequest {
        private String raidName;
    }

    @PostMapping
    public ResponseEntity<?> createPartyRun(@PathVariable UUID partyId,
                                            @RequestBody CreatePartyRunRequest request,
                                            @AuthenticationPrincipal User me) {
        if (me == null) {
            return ResponseEntity.status(401).body("인증이 필요합니다.");
        }
        try {
            PartyRun partyRun = partyRunService.createPartyRun(partyId, request.getRaidName(), me);
